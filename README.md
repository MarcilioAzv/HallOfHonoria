<img width="2169" height="491" alt="Hall_of_Honoria" src="https://github.com/user-attachments/assets/84ae147b-ac03-46f5-8e83-227ac4948ab1" />

<p> </p>
<p> </p>
<p align="center">
  <img src="https://img.shields.io/badge/Godot-4.x-blue?logo=godot-engine">
  <img src="https://img.shields.io/badge/GDScript-Game%20Development-green">
  <img src="https://img.shields.io/badge/Status-Em%20Desenvolvimento-orange">
  <img src="https://img.shields.io/badge/Plataforma-PC-lightgrey">
</p>

## Sobre o Jogo

**Hall of Honoria** é um jogo de ação medieval em terceira pessoa desenvolvido utilizando a **Godot Engine**.

O jogador assume o papel do último guerreiro que permaneceu defendendo o Reino de Honoria após uma invasão bárbara devastadora. Equipado apenas com sua espada, escudo e coragem, ele deverá enfrentar os invasores e impedir a queda definitiva do reino.

O projeto foi desenvolvido com foco em sistemas de combate, inteligência artificial, movimentação 3D e ambientação medieval.

---

##  Controles

  | Tecla | Ação |
  |---------|---------|
  | **W** | Mover para frente |
  | **A** | Mover para esquerda |
  | **S** | Mover para trás |
  | **D** | Mover para direita |
  | **Espaço** | Pular |
  | **Mouse Esquerdo** | Atacar |
  | **Mouse Direito** | Defender com escudo |

---

## Funcionalidades Implementadas

- Combate corpo a corpo com espada e escudo.
- Inteligência Artificial para inimigos.
- Câmera em terceira pessoa.
- Sistema de colisões.
- Ambiente medieval com iluminação dinâmica.

## Capturas de Tela In-Game

### Menu Principal

<p align="center">
  <img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/fb501cbe-5e35-4eb2-9315-96c514861a83" />

</p>

### Gameplay

<p align="center">
  <img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/6f97e412-bff1-4a7f-8f87-612297e429a7" />
  <img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/578ce301-cf64-4ccb-a7f1-0f8535f16c06" />
  <img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/004bf3f9-5aa0-4f2c-9312-829a20853544" />

</p>

### Menu de Pausa

<p align="center">
  <img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/ff2ecf0c-e9f2-40c4-b21f-3b0c22049d7d" />

</p>


---

## Assets Utilizados

Os personagens e alguns recursos visuais utilizados no projeto foram obtidos através do pacote:

**KayKit Adventurers**

https://kaylousberg.itch.io/kaykit-adventurers

Todos os créditos dos modelos 3D pertencem ao autor original do pacote.

### Criação do Mapa

Diferentemente dos personagens, todo o cenário principal de **Hall of Honoria** foi desenvolvido manualmente pela equipe utilizando os recursos nativos da **Godot Engine**.


